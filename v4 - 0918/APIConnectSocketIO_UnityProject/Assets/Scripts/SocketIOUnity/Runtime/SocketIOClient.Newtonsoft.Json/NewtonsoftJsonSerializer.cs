    ...
    public class NewtonsoftJsonSerializer : IJsonSerializer
    {
        ...
        private JsonSerializerSettings NewSettings(ByteArrayConverter converter)
        {
            var settings = new JsonSerializerSettings
            {
                ...
                ObjectCreationHandling = _settings.ObjectCreationHandling,
                ReferenceLoopHandling = _settings.ReferenceLoopHandling,
                StringEscapeHandling = _settings.StringEscapeHandling,
                TypeNameAssemblyFormatHandling = _settings.TypeNameAssemblyFormatHandling,
                TypeNameHandling = _settings.TypeNameHandling
            };
            
            // Copy converters from original settings
            foreach (var originalConverter in _settings.Converters)
            {
                settings.Converters.Add(originalConverter);
            }
